...
try:
    ...
    HAS_LGBM = False
    print("  lightgbm not installed — XGBoost-only ensemble")
...
def build_records_for(
    ...
    spy_rets_series = pd.Series(spy_closes).sort_index().pct_change()
    stock_rets = d["Close"].pct_change()
    # align
    common_idx = d.index
    spy_aligned = spy_rets_series.reindex(common_idx)
    rolling_cov  = stock_rets.rolling(20).cov(spy_aligned)
    rolling_var  = spy_aligned.rolling(20).var().replace(0, 1e-9)
    d["beta_spy20"] = (rolling_cov / rolling_var).fillna(1.0)

    # MACD signal (EMA12 - EMA26, normalised by close price)
    ema12 = d["Close"].ewm(span=12, adjust=False).mean()
    ema26 = d["Close"].ewm(span=26, adjust=False).mean()
    d["macd_signal"] = ((ema12 - ema26) / d["Close"].replace(0, 1e-9)).fillna(0.0)

    sym_rsi14      = d["rsi14"].to_dict()
    sym_beta_spy20 = d["beta_spy20"].to_dict()
    sym_macd       = d["macd_signal"].to_dict()
    ...
    for i, date in enumerate(dates):
        ...
        records.append({
            ...
            "rsi14":            float(sym_rsi14.get(date) or 50.0),
            "beta_spy20":       float(sym_beta_spy20.get(date) or 1.0),
            "macd_signal":      float(sym_macd.get(date) or 0.0),
            "label":            label,
        })
        prev_r1 = r1
...
def train() -> None:
    ...
    if HAS_LGBM:
        ...
        lgbm_acc = float((lgbm_model.predict(X_test) == y_test).mean())
        ...
        ensemble_acc   = float((ensemble_pred == y_test).mean())

        tag_e = "PASS" if ensemble_acc > 0.52 else "INFO"
        print(f"  LightGBM accuracy  : {lgbm_acc*100:.2f}%")
        print(f"  Ensemble accuracy  : {ensemble_acc*100:.2f}%  [{tag_e}]")
    else:
        lgbm_model = None
    ...
    joblib.dump({
        "model":         model,        # XGBoost (primary)
        "lgbm":          lgbm_model,   # LightGBM (None if not installed)
