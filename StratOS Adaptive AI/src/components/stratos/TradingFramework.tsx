...
import { usePortfolio, useWeights, useHealth, useBacktest } from "@/hooks/use-stratos";
import { fmt } from "./live";
import type { Backtest } from "@/lib/stratos-api";
import backtestJson from "@/data/backtest.json";

// Real OPEX-Week Drift backtest results (yfinance 2015–2024), generated by
// `python backtest/opex_backtest.py`. Bundled so the Overview shows real numbers
// even when the live API sidecar isn't running.
const staticBacktest = backtestJson as unknown as Backtest;
// Notional used to render the multiplier equity curve in dollars.
const BACKTEST_NOTIONAL = 10_000;
...
export function TradingFramework() {
  ...
  // Real OPEX-Week Drift backtest. Prefer the live API, fall back to the bundled
  // results so the dashboard shows real numbers even with the sidecar offline.
  const { data: btLive } = useBacktest();
  const bt: Backtest = btLive?.summary ? btLive : staticBacktest;
  const btSummary = bt.summary;
  const btCurve = (bt.equity_curve ?? []).map((p, i) => ({ d: i, v: p.smart, week: p.week_start }));
  const btTrades = bt.trades ?? [];

  // Overview equity curve = backtest multipliers scaled to a $10k notional,
  // starting at the notional itself so the curve runs $10,000 → end value.
  const btDollarCurve = [
    { d: 0, v: BACKTEST_NOTIONAL, week: bt.period?.start ?? "start" },
    ...(bt.equity_curve ?? []).map((p, i) => ({
      d: i + 1,
      v: +(p.smart * BACKTEST_NOTIONAL).toFixed(2),
      week: p.week_start,
    })),
  ];
  const btNetLiq = btDollarCurve.length ? btDollarCurve[btDollarCurve.length - 1].v : BACKTEST_NOTIONAL;
  const btTotalReturn = btSummary?.total_return ?? 0;
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <header className="hidden flex-wrap items-center justify-between gap-3 border-b border-border bg-card px-6 py-3 md:flex">
          <div className="flex flex-wrap items-center gap-3">
            ...
            {tab === "overview" ? (
              <span className="flex items-center gap-1.5 rounded-full border border-primary/30 bg-primary/5 px-2 py-0.5 font-mono text-[10px] text-bull">
          ...
          <div className="flex items-center gap-5 text-right">
            <div>
              <div className="text-[10px] uppercase text-muted-foreground">
                {tab === "overview" ? "Net Liq" : "Equity"}
              </div>
              <div className="text-sm font-semibold text-foreground">
                {tab === "overview" ? fmt.usd(btNetLiq) : fmt.usd(equity)}
              </div>
            </div>
            <div>
              <div className="text-[10px] uppercase text-muted-foreground">
                {tab === "overview" ? "Total Return" : "Day P&amp;L"}
              </div>
              {tab === "overview" ? (
                <div className={`text-sm font-semibold ${btTotalReturn >= 0 ? "text-bull" : "text-bear"}`}>
                  {fmt.signed(btTotalReturn * 100, 2)}%
                </div>
              ) : (
          ...
          {tab === "overview" && (
            <div className="space-y-4 sm:space-y-5">
              <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
                <Stat
                  label="Net Liq"
                  value={fmt.usd(btNetLiq)}
                  delta={`${fmt.signed(btTotalReturn * 100, 2)}% total`}
                  up={btTotalReturn >= 0}
                />
                <Stat
                  label="CAGR"
                  value={fmt.pct(btSummary?.cagr, 1)}
                  delta={`max DD ${fmt.pct(btSummary?.max_drawdown, 1)}`}
                  up={(btSummary?.cagr ?? 0) >= 0}
                />
                <Stat
                  label="Sharpe"
                  value={btSummary?.sharpe != null ? btSummary.sharpe.toFixed(2) : "—"}
                  delta="annualized"
                  up={(btSummary?.sharpe ?? 0) >= 0}
                />
                <Stat
                  label="Win Rate"
                  value={fmt.pct(btSummary?.win_rate, 1)}
                  delta={btSummary?.n_trades != null ? `${btSummary.n_trades} trades` : "—"}
                  up={(btSummary?.win_rate ?? 0) >= 0.5}
                />
              </div>
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-3 lg:gap-5">
                <div className="lg:col-span-2">
                  <Panel
                    title="Equity Curve"
                    subtitle={`${bt.strategy ?? "OPEX-Week Drift"} · ${bt.period?.start ?? "2015"}–${bt.period?.end ?? "2024"} · $${(BACKTEST_NOTIONAL / 1000).toFixed(0)}k notional`}
                  >
                    <ResponsiveContainer width="100%" height={240}>
                      <AreaChart data={btDollarCurve}>
                        <defs>
                        ...
                        <YAxis stroke="#475569" fontSize={11} domain={["dataMin - 200", "dataMax + 200"]} tickFormatter={usdCompact} width={48} />
                        <Tooltip
                          contentStyle={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 8, color: "#e2e8f0" }}
                          formatter={(v: number) => [fmt.usd(v, 2), "equity"]}
                          labelFormatter={(_l, p) => (p && p[0]?.payload?.week) || ""}
                        />
          ...
          {tab === "backtest" && (
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-3 lg:gap-5">
              ...
              <div className="space-y-4 lg:col-span-2 lg:space-y-5">
                <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
                  <Stat label="CAGR" value={fmt.pct(btSummary?.cagr, 1)} delta={btSummary ? "in-market wks" : "—"} up={(btSummary?.cagr ?? 0) >= 0} />
                ...
                <Panel
                  ...
                  subtitle={btSummary ? `${bt.strategy} · ${bt.period?.start}–${bt.period?.end} · yfinance` : "Out-of-sample, net of costs"}
                  right={btSummary ? undefined : <SampleBadge />}
                >
